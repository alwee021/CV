...
alpha_bent = (n3r*2*pi/lamda*10^6*T/u)^2*T/r(k);
%没有气体吸收
...
Ft1 = 4*sqrt(log(2)/log(exp(1))/pi)/theta_d.*exp(-t1.^2./(theta_d.^2)*4*log(2)/log(exp(1))).*exp((R(round(t1./(pi/2).*length(t)))-1)*z./(2.*T.*cot(alpha_1)).*(1+alpha_bent)).*exp(-10^2*0.00002*c(k).*EPLR_t1.*z);
Py1 = dt*cumtrapz(Ft1);
Ft2 = 4*sqrt(log(2)/log(exp(1))/pi)/theta_d.*exp(-t2.^2./(theta_d.^2)*4*log(2)/log(exp(1))).*exp((R(round(t2./(pi/2).*length(t)))-1)*z./(2.*T.*cot(alpha_2)).*(1+alpha_bent)).*exp(-10^2*0.00002*c(k).*EPLR_t2.*z);
Py2 = dt*cumtrapz(Ft2);
Pg = Py1(end) + Py2(end);
