...
%参数列举
lamda = 4.3;  %波长，单位为um
theta_d(k) = 0.22;  %取值？单位？
i_max = length(t);  %取值
delta = 0.02;  %取值，单位为um
d = 0.145;  %膜厚，取值？
T = 0.0005;  %光纤内半径，单位为m
SNR = 10*(m==1) + 20*(m==2) + 40*(m==3) + 100*(m==4);
c(k) = 100 + 2*k;
...
Ft1 = 4*sqrt(log(2)/log(exp(1))/pi)/theta_d(k).*exp(-t1.^2./(theta_d(k).^2)*4*log(2)/log(exp(1))).*sin(t1).*exp((R(round(t1./(pi/2).*length(t))).*exp(-10^2*0.00002*c(k).*EPLR_t1.*t1*2*r(k))-1)*z./(2.*t1.*r(k)));
Py1 = dt*cumtrapz(Ft1);
Ft2 = 4*sqrt(log(2)/log(exp(1))/pi)/theta_d(k).*exp(-t2.^2./(theta_d(k).^2)*4*log(2)/log(exp(1))).*sin(t2).*exp((R(round(t2./(pi/2).*length(t))).*exp(-10^2*0.00002*c(k).*EPLR_t2.*alpha*r(k))-1)*z./(alpha*r(k)));
Py2 = dt*cumtrapz(Ft2);
Pg = Py1(end) + Py2(end);
...
xlabel('气体浓度c/ppm')
ylabel('气体吸收强度/dB')
legend('SNR = 10','SNR = 20','SNR = 40','SNR = 100','SNR趋于无穷大')