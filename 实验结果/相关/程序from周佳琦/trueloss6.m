...
Ft=exp(-t.^2./FWHM./FWHM.*2.77)./FWHM.*sin(t).*(exp((R(round(t.*200+1))-1)/0.002./m.*tan(t).*2)); 
%Ft=exp(-t.^2./FWHM./FWHM.*2.77)./FWHM.*sin(t); 
Sx=dt*cumtrapz(Ft);            %计算区间内曲线下图形面积,为小矩形面积累加得
Sx(end);                %所求定积分值
dt=0.005
t=0:dt:1;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./FWHM./FWHM.*2.77)./FWHM.*sin(t).*(exp(-(1-R(round(t.*200+1)).*exp(-m*0.2.*0.00002.*1000./sin(t)))/0.002./m.*tan(t).*2)); 
...
dt=0.005
t=0:dt:1;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./FWHM./FWHM.*2.77)./FWHM.*sin(t).*(exp(-(1-R(round(t.*200+1)).*exp(-0.2.*0.00002./sin(t)))/0.002.*tan(t).*2));         
Sa=dt*cumtrapz(Ft);            %计算区间内曲线下图形面积,为小矩形面积累加得
Sa(end);                %所求定积分值

dt=0.005
t=0:dt:1;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./FWHM./FWHM.*2.77)./FWHM.*sin(t).*(exp((R(round(t.*200+1))-1)/0.002.*tan(t).*2));        
