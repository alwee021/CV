...
Ft=exp(-t.^2./thetamax./thetamax.*m.*m.*2.25)./thetamax.*m.*sin(t.*m).*(exp((R(round(t.*200.*m+1))-1)/0.002.*tan(t.*m).*1)-exp(-(1-R(round(t.*200.*m+1)).*exp(-1.*0.004.*1./sin(t.*m)))/0.002.*tan(t.*m).*1));         
Sx=dt*cumtrapz(Ft);            %计算区间内曲线下图形面积,为小矩形面积累加得
Sx(end);                %所求定积分值
dt=0.005
t=0:dt:1;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./thetamax./thetamax.*m.*m.*2.25)./thetamax.*m.*sin(t.*m);         
