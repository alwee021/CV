...
Ft=exp(-t.^2./FWHMR./FWHMR.*2.77)./FWHMR.*sin(t).*(exp((R(round(t.*400+1))-1)/0.002./2.*tan(t).*1));         
Sx=dt*cumtrapz(Ft);            %计算区间内曲线下图形面积,为小矩形面积累加得
Sx(end);                %所求定积分值
dt=0.005
t=0:dt:1;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./FWHMR./FWHMR.*2.77)./FWHMR.*sin(t).*(exp(-(1-R(round(t.*400+1)).*exp(-2*0.2.*0.00002.*100./sin(t)))/0.002./2.*tan(t).*1));  
%Ft=exp(-t.^2./FWHMR./FWHMR.*2.77)./FWHMR.*sin(t).*exp(-0.00002.*1000*100./cos(t)); 
