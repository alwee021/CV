...
dt=0.005
t=0:dt:0.17;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./thetamax./thetamax.*2.25)./thetamax.*sin(t).*(exp((R(round(t.*200+1))-1)/0.002.*tan(t).*m)-exp(-(1-R(round(t.*200+1)).*exp(-1.*0.002.*1./sin(t)))/0.002.*tan(t).*m));        
Sx=dt*cumtrapz(Ft);            %计算区间内曲线下图形面积,为小矩形面积累加得
Sx(end);                %所求定积分值
dt=0.005
t=0:dt:0.17;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./thetamax./thetamax.*2.25)./thetamax.*sin(t);         
