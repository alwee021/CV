...
dt=0.005
t=0:dt:0.44;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./thetamax./thetamax.*2.77)./thetamax.*sin(t).*(exp(-(1-R(round(t.*200+1)).*exp(-0.2.*0.00002.*100./sin(t)))/0.002.*tan(t).*m));         
...
dt=0.005
t=0:dt:0.44;                       %数值计算适合于有限区间上,取有限个采样点 
Ft=exp(-t.^2./thetamax./thetamax.*2.77)./thetamax.*sin(t).*(exp(-(1-R(round(t.*200+1)).*exp(-0.2.*0.00002.*99./sin(t)))/0.002.*tan(t).*m));         
...
Sz(end);                %所求定积分值
Sp(k)=((log10(Sx(end)./Sy(end)))-(log10(Sx(end)./Sa(end))))*100
%Sp(k)=((log10(Sx(end)./Sy(end))+log10(1+1/SNR.*Sb(end)./Sx(end))-log10(1+(Sx(end)./Sy(end))/SNR.*Sb(end)./Sx(end)))-(log10(Sx(end)./Sa(end))+log10(1+1/SNR.*Sb(end)./Sx(end))-log10(1+(Sx(end)./Sa(end))/SNR.*Sb(end)./Sx(end))))*100
k=k+1
end
